...
matches = codecs.open('/media/Storage/EAD for Corey/EAD for Corey/idlc.matches.txt', 'r', 'utf-8')

store = HTTP4Store('http://localhost:8080')

r = store.add_graph("http://example.org/idlc", "", "xml")
pprint(r)
count = 0	

for line in matches:
	m = line.split("|")[5].rstrip()
	count += 1
	if m != "None":
		m = m + ".rdf"
		...
		r =  store.append_graph("http://example.org/idlc", raw.encode('utf-8'), "xml")
